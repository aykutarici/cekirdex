...
.kds-head h1 .kds-ic {
    ...
    font-size: 1.05rem; box-shadow: 0 6px 14px -4px rgba(5, 150, 105, .4);
}
.kds-toolbar {
...
.kds-toolbar .kds-actions button.sound.is-on {
    ...
    box-shadow: 0 6px 14px -4px rgba(5, 150, 105, .35);
}
...
.kds-empty i { font-size: 2.75rem; color: var(--c-line-2); display: block; margin-bottom: 14px; }
.kds-empty h2 { font-size: 1.1rem; color: var(--c-text-soft); font-weight: 700; margin-bottom: 6px; }
.kds-empty p { font-size: .92rem; max-width: 360px; margin: 0 auto; line-height: 1.5; }
...
.kds-card.is-new {
    ...
    animation: svc-ring-new 2.5s infinite;
}
.kds-card.is-ready {
    border-left-color: #10b981;
    animation: svc-ring-ready 2s infinite;
}
.kds-card.is-old {
    background: #fffbeb;
    border-color: #fde68a;
...
.kds-card .actions .kds-btn-main {
    ...
    box-shadow: 0 6px 16px -6px rgba(5, 150, 105, .45);
...
.kds-card .actions .kds-btn-main:active:not(:disabled) {
    ...
    box-shadow: 0 2px 8px -4px rgba(5, 150, 105, .55);
}
...
.kds-card.is-busy {
    pointer-events: none;
    outline: 3px solid rgba(5, 150, 105, .4);
    outline-offset: 2px;
    animation: svc-busy-pulse 1.1s ease-in-out infinite;
}
@keyframes svc-busy-pulse {
    0%, 100% { outline-color: rgba(5, 150, 105, .3); }
    50% { outline-color: rgba(5, 150, 105, .65); }
}
...
<div class="kds-page">
    <div class="pp-head kds-head">
        <div>
            <h1>
                <span class="kds-ic"><i class="fas fa-hand-holding"></i></span>
                Servis ekranı
            </h1>
            <div class="sub">Yeni siparişleri mutfağa iletin, hazır olanları masaya teslim edin · ~5 sn yenileme</div>
        </div>
    </div>

    <div class="kds-toolbar">
        <span class="kds-counter"><span class="pulse"></span> <span id="kds-active">0 görev</span></span>
    ...
    <div id="kds-empty" class="kds-empty" style="display:none">
        ...
        <h2>Bekleyen servis görevi yok</h2>
        <p>Yeni sipariş geldiğinde burada listelenir. Mutfağın “hazır” dediği siparişler de yine bu ekranda görünür.</p>
    </div>
</div>
...
function showKdsToast(msg, isError) {
    ...
    kdsToastTimer = setTimeout(() => { el.classList.remove('show'); }, isError ? 3200 : 2200);
}
...
function setCardBusy(orderId, busy) {
    ...
    card.querySelectorAll('.actions button').forEach((btn) => { btn.disabled = !!busy; });
...
function actionsFor(o) {
    if (o.status === 'new')  return [['confirmed','Mutfağa gönder']];
    if (o.status === 'ready') return [['served','Servis edildi']];
    return [];
}

function render(data) {
    const arr = data.orders || [];
    counter.textContent = arr.length + ' görev';
    ...
    grid.innerHTML = arr.map(o => {
        ...
        const actions = `${primaryBtns}${cancelBtn}`;
...
async function advance(id) {
    ...
    try {
        ...
        const msg = d.status === 'confirmed'
            ? 'Mutfağa iletildi — mutfak hazırlayacak'
            : (d.status === 'served' ? 'Servis tamamlandı' : 'Güncellendi');
        showKdsToast(msg);
    } catch (e) {
        console.error(e);
